...
public class OmeroFileHandler implements RemoteFileHandler<OmeroConnection> {
    ...
    public static final Protocol PROTOCOL = new Protocol("ome", 4064, false,
    ...
    public RemoteFile<OmeroConnection> createRemoteFile(final URI uri,
            ...
            final ConnectionMonitor<OmeroConnection> connectionMonitor)
            throws Exception {

        final String path = uri.getPath();
        if ("".equals(path) || "/".equals(path)) { //
            final String name = MessageFormat.format("{0}@{1}",
                    uri.getUserInfo(), uri.getHost());

            return new OmeroRemoteFile(uri, OmeroRemoteFileType.ROOT, 0l,
                    connectionInformation, connectionMonitor, name);
        }
        ...
        if (tokens.length != 3) {
            throw new IllegalArgumentException("The supplied URI: \"" + uri
                    + "\" is not valid. \n"
                    + "You are probably using a node which is not supported with OMERO");
        }

        // tokens[0] is "" because of the leading / in the path
        final OmeroRemoteFileType type =
                EnumUtils.valueForName(tokens[1], OmeroRemoteFileType.values());
        final Long id = Long.parseLong(tokens[2]);

        // Parse the file type out of the uri

        return new OmeroRemoteFile(uri, type, id, connectionInformation,
                connectionMonitor, "file");
    }
